...
public class HoaDonChiTietDAO {
    ...
    private com.example.appbanhang.Database.DatabaseHelper dbHelper;
    public static final String TABLE_NAME = "HoaDonChiTiet";
    public static final String SQL_HOA_DON_CHI_TIET = "CREATE TABLE HoaDonChiTiet (maHDCT INTEGER PRIMARY KEY AUTOINCREMENT, " + "maHoaDon text NOT NULL, maHangHoa text NOT NULL, soLuong INTEGER);";
    public static final String TAG = "HoaDonChiTiet";
    SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");

    public HoaDonChiTietDAO(Context context) {
        dbHelper = new com.example.appbanhang.Database.DatabaseHelper(context);
    ...
    public int inserHoaDonChiTiet(HoaDonChiTiet hd) {
        ...
        values.put("maHangHoa", hd.getHanghoa().getMaHangHoa());
    ...
    public List<HoaDonChiTiet> getAllHoaDonChiTiet() {
        ...
        try {
            while (c.isAfterLast() == false) {
                ...
                ee.setHangHoa(new Hanghoa(c.getString(3), c.getString(4), c.getString(5),
                        c.getString(6), c.getString(7), c.getInt(8), c.getInt(9)));
                ee.setSoLuongMua(c.getInt(10));
                dsHoaDonChiTiet.add(ee);
                Log.d("//=====", ee.toString());
                c.moveToNext();//chuyển tiếp dữ liệu trong danh sách
            }
            c.close();
        } catch (Exception e) {
    ...
    public List<HoaDonChiTiet> getAllHoaDonChiTietByID(String maHoaDon) {
        List<HoaDonChiTiet> dsHoaDonChiTiet = new ArrayList<>();
        String sSQL = "SELECT maHDCT, HoaDon.maHoaDon,HoaDon.ngayMua, "
                + "Hanghoa.maHangHoa,Hanghoa.maTheLoai, Hanghoa.tenhanghoa , Hanghoa.tacGia,Hanghoa.NXB, Hanghoa.giaBia, "
                + "Hanghoa.soLuong,HoaDonChiTiet.soLuong FROM HoaDonChiTiet INNER JOIN HoaDon "
                + "on HoaDonChiTiet.maHoaDon = HoaDon.maHoaDon INNER JOIN Hanghoa on Hanghoa.maHangHoa = HoaDonChiTiet.maHangHoa where HoaDonChiTiet.maHoaDon='"
                + maHoaDon + "'";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        try {
    ...
    public double getDoanhThuTheoNgay() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Hanghoa.giaBia * HoaDonChiTiet.soLuong) as 'tongtien' "
                + "FROM HoaDon INNER JOIN HoaDonChiTiet on HoaDon.maHoaDon = HoaDonChiTiet.maHoaDon "
                + "INNER JOIN Hanghoa on HoaDonChiTiet.maHangHoa = HangHoa.maHangHoa where HoaDon.ngayMua = date('now') GROUP BY HoaDonChiTiet.maHangHoa)tmp";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
    ...
    public double getDoanhThuTheoThang() {
        ...
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Hanghoa.giaBia * HoaDonChiTiet.soLuong) as 'tongtien' "
                + "FROM HoaDon INNER JOIN HoaDonChiTiet on HoaDon.maHoaDon = HoaDonChiTiet.maHoaDon "
                + "INNER JOIN Hanghoa on HoaDonChiTiet.maHangHoa = HangHoa.maHangHoa "
                + "where strftime('%m',HoaDon.ngayMua) = strftime('%m','now') GROUP BY HoaDonChiTiet.maHangHoa)tmp";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();//Chuyển tiếp dữ liệu về đầu danh sách
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
            c.moveToNext();
        }
    ...
    public double getDoanhThuTheoNam() {
        ...
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Hanghoa.giaBia * HoaDonChiTiet.soLuong) as 'tongtien' "
                + "FROM HoaDon INNER JOIN HoaDonChiTiet on HoaDon.maHoaDon = HoaDonChiTiet.maHoaDon "
                + "INNER JOIN Hanghoa on HoaDonChiTiet.maHangHoa = Hanghoa.maHangHoa " +
                "where strftime('%Y',HoaDon.ngayMua) = strftime('%Y','now') GROUP BY HoaDonChiTiet.maHangHoa)tmp";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
