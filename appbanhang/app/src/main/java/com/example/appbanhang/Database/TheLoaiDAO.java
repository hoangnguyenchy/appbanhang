...
public class TheLoaiDAO {
    ...
    public int updateinfoTheLoai(String matl, String tenTL, int vitri) {
        ContentValues values = new ContentValues();
        values.put("tentheloai", tenTL);
        values.put("vitri", vitri);
        int result = db.update(TABLE_NAME, values, "matheloai=?", new String[]{matl});
        if (result == 0) {
            return -1;
        }
        return 1;
